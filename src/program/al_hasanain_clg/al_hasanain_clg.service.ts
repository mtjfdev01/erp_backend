...
export class AlHasanainClgService {
  ...
  async create(dto: CreateAlHasanainClgDto, user: any) {
    ...
    return { success: true, message: 'Al Hasanain CLG record created successfully', data: saved };
  }
  ...
  async findAll(params: {
    ...
    const query = this.repo.createQueryBuilder('a').where('a.is_archived = false');
  ...
  async findOne(id: number) {
    const found = await this.repo.findOne({ where: { id, is_archived: false } });
    if (!found) throw new BadRequestException('Al Hasanain CLG record not found');
    return { success: true, data: found };
  }

  async update(id: number, dto: UpdateAlHasanainClgDto, user: any) {
    const existing = await this.repo.findOne({ where: { id, is_archived: false } });
    if (!existing) throw new BadRequestException('Al Hasanain CLG record not found');

    const patch: Partial<AlHasanainClg> = { updated_by: user };
    if (dto.total_students !== undefined) patch.total_students = dto.total_students;
    if (dto.attendance_percent !== undefined) patch.attendance_percent = dto.attendance_percent;
    ...
    if (dto.fee_collection !== undefined) patch.fee_collection = dto.fee_collection;
    if (dto.active_teachers !== undefined) patch.active_teachers = dto.active_teachers;

    await this.repo.update(id, patch);
    const updated = await this.repo.findOne({ where: { id, is_archived: false } });
    return { success: true, message: 'Al Hasanain CLG record updated successfully', data: updated };
  }
  ...
  async remove(id: number) {
    ...
    return { success: true, message: 'Al Hasanain CLG record deleted successfully' };
  }
}

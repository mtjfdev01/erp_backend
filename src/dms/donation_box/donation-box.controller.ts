...
export class DonationBoxController {
   ...
   */
  private async checkGeographicAccess(userId: number, cityId: number): Promise<void> {
    const assignedCityIds = await this.donationBoxService.resolveUserGeographyCityIds(userId);
  ...
  async create(@Body() createDonationBoxDto: CreateDonationBoxDto, @Res() res: Response, @CurrentUser() currentUser: any) {
    try {
      ...
      if (createDonationBoxDto.city_id) {
        await this.checkGeographicAccess(currentUser.id, createDonationBoxDto.city_id);
      }
  ...
  async findAll(
    ...
    try {
      ...
      const assignedCityIds = await this.donationBoxService.resolveUserGeographyCityIds(currentUser?.id);
  ...
  async findActiveByRegion(@Param('region') region: string, @Res() res: Response, @CurrentUser() currentUser?: any) {
    try {
  ...
  @RequiredPermissions(['fund_raising.donation_box.view', 'super_admin', 'fund_raising_manager', 'fund_raising_user'])
  async findByKeyNumber(@Param('key_number') key_number: string, @Res() res: Response, @CurrentUser() currentUser?: any) {
    try {

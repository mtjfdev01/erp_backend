...
import { ProgressStatus, TrackerOverallStatus, ParentEntityType } from '../common/progress-tracking.enum';
...
export class ProgressTrackersService {
  ...
  private deriveOverallStatus(steps: ProgressTrackerStep[]): TrackerOverallStatus {
    const active = steps.filter((s) => !s.is_archived);
    if (active.length === 0) return TrackerOverallStatus.PENDING;
    if (active.every((s) => s.status === ProgressStatus.CANCELLED)) return TrackerOverallStatus.CANCELLED;
    if (active.every((s) => s.status === ProgressStatus.COMPLETED || s.status === ProgressStatus.SKIPPED)) return TrackerOverallStatus.COMPLETED;
    if (active.some((s) => s.status === ProgressStatus.IN_PROGRESS || s.status === ProgressStatus.COMPLETED)) return TrackerOverallStatus.IN_PROGRESS;
  ...
  private validateTransition(from: ProgressStatus, to: ProgressStatus) {
    const allowed: Record<ProgressStatus, ProgressStatus[]> = {
      [ProgressStatus.PENDING]: [ProgressStatus.IN_PROGRESS, ProgressStatus.COMPLETED, ProgressStatus.SKIPPED, ProgressStatus.CANCELLED],
      [ProgressStatus.IN_PROGRESS]: [ProgressStatus.COMPLETED, ProgressStatus.CANCELLED],
      [ProgressStatus.COMPLETED]: [],
      [ProgressStatus.SKIPPED]: [],
      [ProgressStatus.CANCELLED]: [],
    };
    if (!allowed[from]?.includes(to)) {
      throw new BadRequestException(`Invalid step transition: ${from} -> ${to}`);
    }
  }

  async createTrackerFromTemplate(dto: CreateTrackerDto, currentUser?: any) {
    const template = await this.templatesRepo.findOne({ where: { id: dto.template_id, is_archived: false } });
    if (!template) throw new NotFoundException('Template not found');

    const donationId = dto.donation_id ?? null;
    if (donationId) {
      const existing = await this.trackersRepo.findOne({ where: { donation_id: donationId, is_archived: false } });
      if (existing) throw new ConflictException('Tracker already exists for this donation');
    }

    const trackerEntity = this.trackersRepo.create({
      template_id: template.id,
      donation_id: donationId,
      parent_type: dto.parent_type || (donationId ? ParentEntityType.DONATION : null),
    ...
    const tracker = Array.isArray(savedTracker) ? savedTracker[0] : savedTracker;

    const templateSteps = await this.templateStepsRepo.find({
      where: { template_id: template.id, is_archived: false },
      order: { step_order: 'ASC' },
    });
    if (!templateSteps.length) throw new BadRequestException('Template has no steps');

    const stepRows: DeepPartial<ProgressTrackerStep>[] = templateSteps.map((ts) => ({
  ...
  async listTrackers(options?: { page?: number; pageSize?: number; search?: string; template_id?: number; donation_id?: number }) {
    ...
    if (options?.template_id) qb.andWhere('t.template_id = :templateId', { templateId: options.template_id });
    if (options?.donation_id) qb.andWhere('t.donation_id = :donationId', { donationId: options.donation_id });

    if (options?.search) {
      qb.andWhere('(template.name ILIKE :q OR template.code ILIKE :q OR CAST(t.donation_id AS text) ILIKE :q)', { q: `%${options.search}%` });
    }
  ...
  async getTrackerDetail(trackerId: number) {
    ...
    tracker.steps = (tracker.steps || []).filter((s) => !s.is_archived).sort((a, b) => a.step_order - b.step_order);
    return tracker;
  }

  async getTrackerByDonationId(donationId: number) {
    const tracker = await this.trackersRepo.findOne({ where: { donation_id: donationId, is_archived: false } });
    if (!tracker) throw new NotFoundException('Tracker not found for donation');
    return this.getTrackerDetail(tracker.id);
  }

  async updateTracker(trackerId: number, dto: UpdateTrackerDto, currentUser?: any) {
    const tracker = await this.trackersRepo.findOne({ where: { id: trackerId, is_archived: false } });
    if (!tracker) throw new NotFoundException('Tracker not found');
    await this.trackersRepo.update(trackerId, { ...dto, updated_by: currentUser?.id === -1 ? null : currentUser } as any);
  ...
  async generateOrRegenerateToken(trackerId: number, currentUser?: any) {
    ...
    await this.trackersRepo.update(trackerId, { public_tracking_token: token, updated_by: currentUser?.id === -1 ? null : currentUser } as any);
    return { token };
  }
  ...
  async updateStep(stepId: number, dto: UpdateTrackerStepStatusDto, currentUser?: any) {
    ...
    if (dto.donor_visible !== undefined) updateData.donor_visible = dto.donor_visible;

    if (dto.status === ProgressStatus.IN_PROGRESS && !step.started_at) updateData.started_at = now;
    ...
    const tracker = await this.trackersRepo.findOne({ where: { id: step.tracker_id }, relations: ['steps', 'donation', 'donation.donor'] as any });
    if (tracker) {
      const overall = this.deriveOverallStatus((tracker.steps || []).filter((s) => !s.is_archived));
      await this.trackersRepo.update(tracker.id, { overall_status: overall } as any);
    }

    const updatedStep = await this.trackerStepsRepo.findOne({ where: { id: stepId }, relations: ['evidence'] });

    if (dto.status === ProgressStatus.COMPLETED) {
      try {
        const templateStep = step.template_step_id
          ? await this.templateStepsRepo.findOne({ where: { id: step.template_step_id } })
          : null;
        if (templateStep?.notify_donor_on_complete) {
          const trackerFull = await this.trackersRepo.findOne({ where: { id: step.tracker_id }, relations: ['donation', 'donation.donor'] as any });
  ...
  async addEvidence(stepId: number, dto: AddEvidenceDto, currentUser?: any) {
    const step = await this.trackerStepsRepo.findOne({ where: { id: stepId, is_archived: false } });
    if (!step) throw new NotFoundException('Tracker step not found');
  ...
  async updateEvidence(evidenceId: number, dto: Partial<AddEvidenceDto>, currentUser?: any) {
    const existing = await this.evidenceRepo.findOne({ where: { id: evidenceId, is_archived: false } });
    if (!existing) throw new NotFoundException('Evidence not found');
    await this.evidenceRepo.update(evidenceId, { ...dto, updated_by: currentUser?.id === -1 ? null : currentUser } as any);
  ...
  async archiveEvidence(evidenceId: number, currentUser?: any) {
    const existing = await this.evidenceRepo.findOne({ where: { id: evidenceId, is_archived: false } });
    if (!existing) throw new NotFoundException('Evidence not found');
    await this.evidenceRepo.update(evidenceId, { is_archived: true, updated_by: currentUser?.id === -1 ? null : currentUser } as any);

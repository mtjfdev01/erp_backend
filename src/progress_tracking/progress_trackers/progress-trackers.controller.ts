...
export class ProgressTrackersController {
  ...
  async create(@Body() dto: CreateTrackerDto, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.createTrackerFromTemplate(dto, user);
    return res.status(HttpStatus.CREATED).json({ success: true, message: 'Tracker created', data });
  }
  ...
  async list(
    ...
    return res.status(HttpStatus.OK).json({ success: true, message: 'Trackers retrieved', ...data });
  }
  ...
  async detail(@Param('id') id: string, @Res() res: Response) {
    ...
    return res.status(HttpStatus.OK).json({ success: true, message: 'Tracker retrieved', data });
  }

  @Get('by-donation/:donationId')
  @RequiredPermissions(['fund_raising.donations.view', 'super_admin', 'fund_raising_manager', 'fund_raising_user'])
  async byDonation(@Param('donationId') donationId: string, @Res() res: Response) {
  ...
  async update(@Param('id') id: string, @Body() dto: UpdateTrackerDto, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.updateTracker(+id, dto, user);
    return res.status(HttpStatus.OK).json({ success: true, message: 'Tracker updated', data });
  }

  @Post(':id/token')
  @RequiredPermissions(['fund_raising.donations.update', 'super_admin', 'fund_raising_manager'])
  async regenToken(@Param('id') id: string, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.generateOrRegenerateToken(+id, user);
    return res.status(HttpStatus.OK).json({ success: true, message: 'Token generated', data });
  }
  ...
  async listSteps(@Param('id') id: string, @Res() res: Response) {
    ...
    return res.status(HttpStatus.OK).json({ success: true, message: 'Tracker steps retrieved', data });
  }

  @Patch('steps/:stepId')
  @RequiredPermissions(['fund_raising.donations.update', 'super_admin', 'fund_raising_manager'])
  async updateStep(@Param('stepId') stepId: string, @Body() dto: UpdateTrackerStepStatusDto, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.updateStep(+stepId, dto, user);
    return res.status(HttpStatus.OK).json({ success: true, message: 'Tracker step updated', data });
  }

  @Post('steps/:stepId/evidence')
  @RequiredPermissions(['fund_raising.donations.update', 'super_admin', 'fund_raising_manager'])
  async addEvidence(@Param('stepId') stepId: string, @Body() dto: AddEvidenceDto, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.addEvidence(+stepId, dto, user);
    return res.status(HttpStatus.CREATED).json({ success: true, message: 'Evidence added', data });
  }

  @Patch('evidence/:evidenceId')
  @RequiredPermissions(['fund_raising.donations.update', 'super_admin', 'fund_raising_manager'])
  async updateEvidence(@Param('evidenceId') evidenceId: string, @Body() dto: Partial<AddEvidenceDto>, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.updateEvidence(+evidenceId, dto, user);
    return res.status(HttpStatus.OK).json({ success: true, message: 'Evidence updated', data });
  }

  @Delete('evidence/:evidenceId')
  @RequiredPermissions(['fund_raising.donations.update', 'super_admin', 'fund_raising_manager'])
  async archiveEvidence(@Param('evidenceId') evidenceId: string, @Res() res: Response, @CurrentUser() user: any) {
    const data = await this.service.archiveEvidence(+evidenceId, user);
    return res.status(HttpStatus.OK).json({ success: true, message: 'Evidence archived', data });
  }
}
...
export class ProgressPublicController {
  ...
  async getPublic(@Param('token') token: string, @Res() res: Response) {
    ...
    return res.status(HttpStatus.OK).json({ success: true, message: 'Public tracking retrieved', data });
  }
}
